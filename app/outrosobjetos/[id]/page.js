...
export default function OutrosObjetosDetalhes() {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function fetchRecord() {
      try {
        const res = await fetch(`/api/outrosobjetos/${id}`);
        if (res.ok) {
  ...
  async function handleUpdate(formData) {
    ...
    try {
      ...
      if (res.ok) {
        setNotificationMessage("Outro Objeto atualizado com sucesso!");
  ...
  return (
    <div>
      <OutrosObjetosForm
        ...
        isUpdating={true}
        title="Detalhes e Atualização do Outro Objeto:"
      />
