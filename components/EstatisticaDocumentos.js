...
export default function EstatisticaDocumentos() {
  ...
  useEffect(() => {
    async function fetchRecords() {
      try {
        setLoading(true);
        const res = await fetch('/api/documentos?limit=1000');
        if (!res.ok) {
          throw new Error('Falha ao buscar dados de documentos');
        }
  ...
  const processarDados = () => {
    // 1. Total de documentos apreendidos
  ...
  const gerarPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(18);
      pdf.text('Relatório de Estatísticas - Documentos', pdfWidth / 2, 15, { align: 'center' });
      ...
      // Total de documentos apreendidos
      pdf.text(`Total de documentos apreendidos: ${totalApreendidosCount}`, 15, y);
      y += 8;
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-bold">Estatísticas de Documentos</h2>
        <button 
          onClick={gerarPDF} 
          disabled={gerando}
          ...
          <div className="flex flex-wrap w-full gap-6 mb-6">
            ...
            <div className="flex-1 min-w-[45%] h-[300px] bg-c_deep_middle_black rounded-lg shadow-lg p-4 border border-gray-700">
              <div className="flex flex-col h-full justify-center items-center">
                <h2 className="text-xl font-semibold mb-2">Documentos Apreendidos</h2>
                <div className="text-5xl font-bold text-blue-400 mb-2">{totalApreendidosCount}</div>
                <p className="text-sm text-center text-gray-400">Total de documentos com status "apreendido"</p>
              </div>
            </div>
