...
export default function EstatisticaBelicos() {
  const [belicos, setBelicos] = useState([]);
  ...
  useEffect(() => {
    async function fetchBelicos() {
      try {
        setLoading(true);
        const res = await fetch('/api/belicos?limit=1000');
        if (!res.ok) {
          throw new Error('Falha ao buscar dados de bélicos');
        }
        const data = await res.json();
        setBelicos(data.records || []);
      } catch (err) {
        console.error('Erro ao buscar bélicos:', err);
  ...
  const processarDados = () => {
    // 1. Total de bélicos apreendidos no depósito ou cartório
    const apreendidosDepositoCartorio = belicos.filter(b =>
      b.status === 'apreendido' &&
      (b.destino === 'depósito' || b.destino === 'cartório')
    );
    ...
    const tiposCount = {};
    belicos.forEach(b => {
      const tipo = b.customTipo || b.tipo;
      const quantidade = parseFloat(b.quantidade) || 1;
      const unidade = b.unidMedida || 'un';
    ...
    belicos.forEach(b => {
      if (b.marca) {
        if (!marcasModelos[b.marca]) {
          marcasModelos[b.marca] = {};
        }
        const modelo = b.modelo || 'Não especificado';
        marcasModelos[b.marca][modelo] = (marcasModelos[b.marca][modelo] || 0) + 1;
      }
    });
    ...
    belicos.forEach(b => {
      if (b.data) {
        const data = new Date(b.data).toLocaleDateString();
    ...
    belicos.forEach(b => {
      if (b.procedimento) {
        procedimentosCount[b.procedimento] = (procedimentosCount[b.procedimento] || 0) + 1;
      }
    });
  ...
  const gerarPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(18);
      pdf.text('Relatório de Estatísticas - Bélicos', pdfWidth / 2, 15, { align: 'center' });
      ...
      // Total de bélicos apreendidos
      pdf.text(`Total de bélicos apreendidos no depósito/cartório: ${totalApreendidosCount}`, 15, y);
      y += 8;
      ...
      // Salva o PDF
      pdf.save('relatorio-belicos.pdf');
    } catch (err) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-bold">Estatísticas de Bélicos</h2>
        <button 
          onClick={gerarPDF} 
          disabled={gerando}
          ...
          <div className="flex flex-wrap w-full gap-6 mb-6">
            ...
            <div className="flex-1 min-w-[45%] h-[300px] bg-c_deep_middle_black rounded-lg shadow-lg p-4 border border-gray-700">
              <div className="flex flex-col h-full justify-center items-center">
                <h2 className="text-xl font-semibold mb-2">Bélicos Apreendidos no Depósito/Cartório</h2>
                <div className="text-5xl font-bold text-blue-400 mb-2">{totalApreendidosCount}</div>
                <p className="text-sm text-center text-gray-400">Total de bélicos com status "apreendido" e destino "depósito" ou "cartório"</p>
              </div>
            </div>
